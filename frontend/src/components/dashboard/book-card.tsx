...
export function BookCard({ book, canManageBooks, onCheckoutClick, onCheckinClick, onEditClick, onDeleteClick }: BookCardProps) {
  return (
    <Card className="flex flex-col h-full overflow-hidden transition-all duration-300 hover:shadow-2xl animate-in fade-in zoom-in-95">
        ...
        <div className="relative aspect-[3/4]">
          ...
          />
           <div className="absolute top-2 right-2 flex items-center gap-2">
            <Badge variant={book.status === 'lent' ? 'destructive' : 'secondary'} className="capitalize">
              {book.status}
            </Badge>
          </div>
        </div>
      ...
      <CardContent className="p-4 flex-grow">
        <div className="flex justify-between items-start">
            <CardTitle className="text-lg font-headline mb-2 flex-1 pr-2">{book.module}</CardTitle>
            {canManageBooks && (
              <DropdownMenu>
        ...
        <div className="text-sm text-muted-foreground space-y-1">
          <p>Lesson {book.lessonNumber}, Copy {book.copyNumber}</p>
          <p>Band: {book.band}</p>
      ...
      <CardFooter className="p-4 pt-0">
        {book.status === 'available' ? (
          <Button className="w-full bg-accent text-accent-foreground hover:bg-accent/90" onClick={() => onCheckoutClick(book)}>
            <LogIn className="mr-2 h-4 w-4" /> Check Out
          </Button>
        ) : (
          <Button variant="outline" className="w-full" onClick={() => onCheckinClick(book.id)}>
